...
# NOTE: This script is optimized for the Humboldt Current System.
# UNIX-style globbing
import glob
...
def main():
    fileDir = '/glade/p/work/rbrady/EBUS_BGC_Variability/FG_CO2/HumCS/'
    ...
    ds = ds.where(ds['TLAT'] >= -16).where(ds['TLAT'] <= -6)
    ...
    df_ipo = pd.DataFrame(index=index, columns=columns)
    ...
    for idx in np.arange(0, 34, 1):
        ...
        ts4 = ds_cvdp['ipo'][idx, 11::].values
        ts5 = ds_cvdp['sam'][idx, 11::].values
        ...
        df_ipo = linear_regression(df_ipo, idx, ts4, ts1)
        df_sam = linear_regression(df_sam, idx, ts5, ts1)
    df_enso.to_csv('smoothed_fgco2_vs_enso_humcs')
    df_pdo.to_csv('smoothed_fgco2_vs_pdo_humcs')
    df_ipo.to_csv('smoothed_fgco2_vs_ipo_humcs')
    df_sam.to_csv('smoothed_fgco2_vs_sam_humcs')
